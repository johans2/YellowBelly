...
public class GvrArmModel : MonoBehaviour {
  ...
  private static readonly Vector3 defaultShoulderRight = new Vector3(0.19f, -0.19f, -0.03f);

  /// The range of movement from the elbow position due to accelerometer (meters).
  private static readonly Vector3 elbowMinRange = new Vector3(-0.05f, -0.1f, 0.0f);
  private static readonly Vector3 elbowMaxRange = new Vector3(0.05f, 0.1f, 0.2f);
  ...
  public static GvrArmModel Instance {
    get {
      if (instance == null) {
        GvrController controller = GvrController.Instance;
        if (controller != null) {
          instance = controller.GetComponent<GvrArmModel>();
        }
      }
  ...
  void Start() {
    ...
    // Make sure the IMU sensors are enabled.
    if (!controller.enableAccel || !controller.enableGyro) {
      Debug.LogWarning("Enable Accel and Gyro in GvrController to use GvrArmModel.");
      enabled = false;
    }
  ...
  private void UpdateHandedness() {
    ...
    shoulderPosition = Vector3.Scale(defaultShoulderRight, handedMultiplier);
  }
  ...
  private void TransformElbow() {
    ...
    if (useAccelerometer) {
      ...
      elbowOffset.x = Mathf.Clamp(elbowOffset.x, elbowMinRange.x, elbowMaxRange.x);
      elbowOffset.y = Mathf.Clamp(elbowOffset.y, elbowMinRange.y, elbowMaxRange.y);
      elbowOffset.z = Mathf.Clamp(elbowOffset.z, elbowMinRange.z, elbowMaxRange.z);
    }
  }
  ...
  private void ApplyArmModel() {
    ...
    elbowPosition = new Vector3(0.195f, -0.5f + addedElbowHeight, -0.075f + addedElbowDepth);
    elbowPosition = Vector3.Scale(elbowPosition, handedMultiplier) + elbowOffset;
    wristPosition = Vector3.Scale(new Vector3(0.0f, 0.0f, 0.25f), handedMultiplier);
    Vector3 armExtensionOffset = Vector3.Scale(new Vector3(-0.13f, 0.14f, 0.08f), handedMultiplier);
  ...
  private void UpdatePointer() {
    ...
    pointerPosition = wristPosition + wristRotation * new Vector3(0.0f, -0.009f, 0.099f);
